 ...
 extends TableView<List<Object>>
{
  ...
  private void setTableHeight(TableViewSkin<?> tvs)
  {
    ...
    TableHeaderRow thr = tvs.getTableHeaderRow();
    _dHeaderHeight = thr.getInsets().getTop() + thr.getInsets().getBottom() +
        thr.getLayoutBounds().getHeight();
    dTableHeight += _dHeaderHeight;
    // System.out.println("Header has height "+_dHeaderHeight);
    /* rows */
    double dRowsHeight = 0.0;
    for (int iRow = 0; iRow < iRows; iRow++)
    {
      double dRowHeight = 0.0;
      List<ObjectTableCell> listTableCells = _llTableCells.get(iRow);
      for (int iColumn = 0; iColumn < listTableCells.size(); iColumn++)
      {
        ObjectTableCell otc = listTableCells.get(iColumn);
        double dCellHeight = otc.getMinHeight();
        if (dRowHeight < dCellHeight)
          dRowHeight = dCellHeight;
      }
      // System.out.println("Row "+iRow+" has height: "+dRowHeight);
      dRowsHeight += dRowHeight;
    }
